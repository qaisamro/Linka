...
const RoleBadge = ({ role }) => (
  <span className={`badge-pill text-[10px] font-black px-2 py-0.5 ${role === 'admin'
...
const StatusBadge = ({ isActive }) => (
  <span className={`badge-pill text-[10px] font-bold px-2 py-0.5 ${isActive
    ? 'bg-[#F9F5F0] text-[#344F1F] border border-[#F2EAD3]'
    : 'bg-[#F9F5F0] text-[#344F1F] border border-[#F2EAD3]'
    }`}>
    {isActive ? '🟢 نشط' : '🔴 معطّل'}
                  ...
                  {users.map((u, idx) => {
                    ...
                    return (
                      <motion.tr
                        ...
                        className={`group transition-colors ${u.is_active ? 'hover:bg-[#F9F5F0]/30' : 'bg-[#F9F5F0]/30 hover:bg-[#F9F5F0]/60'
                          }`}
                      >
                        {/* User identity */}
                        <td className="px-5 py-3.5">
                          <div className="flex items-center gap-3">
                            <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-[#F9F5F0] font-black text-sm flex-shrink-0 shadow-sm overflow-hidden ${u.is_active ? 'bg-hero-gradient' : 'bg-[#F2EAD3]'
                              }`}>
                              {u.avatar_url ? (
                            ...
                            {!isSelf && !isAdmin && (
                              <>
                                ...
                                <motion.button
                                  ...
                                  className={`p-2 rounded-xl transition-all duration-200 ${u.is_active
